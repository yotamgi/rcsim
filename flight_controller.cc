...
ServoData HeliFlightController::translate(const ServoData& servo_data, float time_delta) {
    ...
    if (servo_data.channels[HELI_CHANNEL_THROTTLE] < -0.7) {
        ...
        zero_servo_data.channels[HELI_CHANNEL_THROTTLE] = servo_data.channels[HELI_CHANNEL_THROTTLE];
