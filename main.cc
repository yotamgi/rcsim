...
void add_banana(irr::scene::ISceneManager *smgr,
  ...
  banana_node->setMaterialTexture(
      0, driver->getTexture("media/banana/textures/rgb.jpeg"));
...
int main() {
  ...
  */
  ...
  IrrlichtDevice *device =
      createDevice(video::EDT_OPENGL, dimension2d<u32>(1600, 1200), 32, false,
                   true, false, &receiver);
  ...
  */
  IMesh *mesh =
      smgr->getMesh("media/BasketballStadium/source/63BasketBallZemin.obj");
  if (!mesh) {
    device->drop();
    return 1;
  }
  ...
  node->setMaterialTexture(
      0, driver->getTexture(
             "media/BasketballStadium/textures/BasketZemin_Color.png"));
  for (unsigned int i = 0; i < node->getMaterialCount(); i++) {
    node->getMaterial(i).AmbientColor.set(255, 255, 255, 255);
  }

  add_banana(smgr, driver, core::vector3df(-0.5, 0.05, 0.2),
             core::vector3df(90, 73, 0));
  add_banana(smgr, driver, core::vector3df(0.9, 0.05, 0.3),
             core::vector3df(90, 40, 0));
  add_banana(smgr, driver, core::vector3df(0.4, 0.05, 0.0),
             core::vector3df(90, 0, 0));

  // Add skybox
  smgr->addSkyBoxSceneNode(driver->getTexture("media/skybox/irrlicht2_up.jpg"),
  ...
  */
  ...
  Configuration model_conf = MODEL_CONFIGURATIONS[1].create(driver, smgr);
