...
Configuration create_rc_bell_heli(irr::video::IVideoDriver* driver, irr::scene::ISceneManager* smgr) {
    ...
    std::shared_ptr<BaseHeli> heli_model = std::make_shared<RcBellHeli>(smgr, driver);
    std::shared_ptr<HeliFlightController> flight_controller = std::make_shared<HeliFlightController>(heli_model);

    Configuration conf;
    conf.model = heli_model;
    conf.controls = std::make_shared<HeliControls>(
            flight_controller, throttle_curves, lift_curves
    );
...
std::vector<ModelConfiguration> MODEL_CONFIGURATIONS = {
    {
        .name = std::string("RC Bell Heli"),
        .create = create_rc_bell_heli
    }
};
