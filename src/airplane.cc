...
void print_matrix(const std::string str, engine::mat4 &mat) {
...
static engine::vec3 advection_force(const engine::vec3 &airflow,
                                    const engine::vec3 &surface_normal,
                                    float surface_area) {
  engine::vec3 airflow_direction = engine::vec3(airflow).Normalize();
...
AppliedForce PointAirFoil::get_force(const engine::vec3 &airflow) {
  ...
  engine::vec3 lift = advection_force(airflow, m_params.normal, m_params.area);

  // The drag;
  engine::vec3 drag = advection_force(airflow, m_params.wing_airflow_direction,
                                      m_params.drag_area);
  ...
  // The total force.
  engine::vec3 force = drag + lift + flap + wing_effect;
...
void PointAirFoil::init_ui(engine::RaylibDevice *device,
                           std::shared_ptr<engine::Model> parent) {
  auto sphere_model = device->create_sphere(0.03f, 16, 16, parent);
  sphere_model->set_transform(engine::mat4::Translate(
      m_params.position_in_airplane.x, m_params.position_in_airplane.y,
      m_params.position_in_airplane.z));
...
RectangularAirFoil::RectangularAirFoil(const RectangularAirFoil::Params &params)
    : m_params(params) {
  engine::mat4 rotoation_mat =
      engine::mat4::RotateXYZ(m_params.rotation_angles / 180 * M_PI);
  engine::vec3 normal(0, 1, 0), wing_direction(0, 0, 1), along_wing(1, 0, 0);
  normal = (raylib::Matrix)rotoation_mat * normal;
  wing_direction = rotoation_mat * wing_direction;
  along_wing = rotoation_mat * along_wing;

  engine::vec3 wing_start =
  ...
  for (int i = 0; i < m_params.num_points; i++) {
    engine::vec3 position = wing_start + i * points_interval * along_wing;
...
RectangularAirFoil::calc_force(const engine::vec3 &wind_in_airplane,
...
void RectangularAirFoil::set_flap(float value) {
  ...
  engine::mat4 rotation =
      engine::mat4::Rotate(m_along_wing_direction, -angle_radians);
  engine::vec3 flap_normal = rotation * m_wing_normal;
  for (auto point_airfoil : m_point_airfoils) {
    point_airfoil->set_flap_normal(flap_normal);
  }
}

void RectangularAirFoil::init_ui(engine::RaylibDevice *device,
                                 std::shared_ptr<engine::Model> parent) {
  auto airofoil_model = device->create_cube(
      m_params.x_length, m_params.y_thickness, m_params.z_width, parent);
  airofoil_model->set_transform(
      engine::mat4::RotateXYZ(m_params.rotation_angles / 180 * M_PI) *
      engine::mat4::Translate(m_params.position_in_airplane.x,
                              m_params.position_in_airplane.y,
                              m_params.position_in_airplane.z));
  for (auto point_airfoil : m_point_airfoils) {
    point_airfoil->init_ui(device, parent);
  }
}
...
AppliedForce Propellant::calc_force(const engine::vec3 &wind_in_airplane,
                                    const engine::vec3 &velocity_in_airplane) {
  engine::vec3 airflow = wind_in_airplane - velocity_in_airplane;
  ...
  engine::vec3 force = -force_magnitude * m_params.direction_in_airplane;
...
Airplane::Airplane(const Airplane::Params &params, engine::RaylibDevice *device)
    : m_ui_node(device->create_empty()), m_params(params) {
...
void Airplane::init_ui(engine::RaylibDevice *device) {
  if (!m_params.show_skeleton)
    return;

  for (const auto &airfoil : m_airfoils) {
    airfoil->init_ui(device, m_ui_node);
  }

  // for (const auto &prop : m_propellants) {
  //   prop->init_ui(device, m_ui_node);
  // }

  for (const auto &tp : m_params.touchpoints_in_airplane) {
    auto point_model = device->create_sphere(0.05f, 16, 16, m_ui_node);
    point_model->set_transform(
        engine::mat4::Translate(tp.pos.x, tp.pos.y, tp.pos.z));
  }
}

void Airplane::update(double time_delta, const engine::vec3 &wind_speed) {
  // Update the servos.
  ...
  engine::vec3 airfoil_forces_in_airplane(0, 0, 0);
  engine::vec3 airfoil_torque_in_airplane(0, 0, 0);
  engine::vec3 wind_in_airplane = m_rotation_in_world.Transpose() * wind_speed;
  engine::vec3 velocity_in_ariplane =
      m_rotation_in_world.Transpose() * m_velocity_in_world;
  for (const auto &airfoil : m_airfoils) {
  ...
  engine::vec3 airfoil_force_in_world =
      m_rotation_in_world * airfoil_forces_in_airplane;
  // Calculate the force from the propellants.
  engine::vec3 prop_forces_in_airplane(0, 0, 0);
  engine::vec3 prop_torque_in_airplane(0, 0, 0);
  for (const auto &prop : m_propellants) {
  ...
  engine::vec3 propellant_force_in_world =
      m_rotation_in_world * prop_forces_in_airplane;

  // Gravity force.
  engine::vec3 gravity_in_world =
      engine::vec3(0, -m_params.mass * GRAVITY_CONSTANT, 0);

  // Update position and velocity according to the forces.
  engine::vec3 total_force = airfoil_force_in_world +
  ...
  engine::vec3 total_torque_in_airplane = airfoil_torque_in_airplane +
                                          prop_torque_in_airplane +
  ...
  m_angular_velocity_in_airplane +=
      ...
      m_params.moi;
  engine::vec3 angulal_velocity_in_world =
      m_rotation_in_world * m_angular_velocity_in_airplane;
  ...
  m_airspeed = (m_velocity_in_world - wind_speed).Length();

  if (std::isnan(m_position_in_world.x) || std::isnan(m_position_in_world.y) ||
      std::isnan(m_position_in_world.z)) {
    throw std::runtime_error("Airplane position is NaN");
  }
}
...
void Airplane::add_force(unsigned int touchpoint_index,
  ...
  m_external_torque_in_airplane +=
      m_params.touchpoints_in_airplane[touchpoint_index].pos.CrossProduct(
          m_rotation_in_world.Transpose() * force);
}
...
std::vector<Airplane::TouchPoint> Airplane::get_touchpoints_in_world() const {
  ...
  for (size_t tp_index = 0; tp_index < m_params.touchpoints_in_airplane.size();
    ...
    tp.pos = m_position_in_world + m_rotation_in_world * touchpoint_in_body.pos;
    tp.vel = m_velocity_in_world +
             m_rotation_in_world * m_angular_velocity_in_airplane.CrossProduct(
                                       touchpoint_in_body.pos);
    engine::mat4 friction_in_body = touchpoint_in_body.friction_coeff;
...
void Airplane::update_ui() {
  m_ui_node->set_transform(m_rotation_in_world *
                           engine::mat4::Translate(m_position_in_world.x,
                                                   m_position_in_world.y,
                                                   m_position_in_world.z));
}
...
Airplane::Telemetry Airplane::get_telemetry() const {
  ...
  telemetry.velocity_magnitude = m_velocity_in_world.Length();
